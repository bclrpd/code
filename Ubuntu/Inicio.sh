...
if [ -z $(nmcli d |grep "connected" -w | awk '{print $2}') ] ; then	 #Revisa si no hay conexion
	if [ $(nmcli n) = "disabled" ] ;then #Revisa si Networking esta desactivado, si es asi lo activa 
		nmcli n on
		sleep 5
	fi
	if [ -z $(nmcli d |grep "connected" -w | awk '{print $2}') ] ;then #Revisa si aun no hay conexion
		if [ $(nmcli r wifi) = "disabled" ] ; then #Revisa si WIFI esta desactivado, si es asi lo activa 
			nmcli r wifi on
			sleep 5
			if [ -z $(nmcli d |grep "connected" -w | awk '{print $2}') ] ; then #Revisa si aun no hay conexion
				nmcli c up "Wi-Fi connection 1" &
			fi
		else
			nmcli c up "Wi-Fi connection 1" &
		fi
...
for i in {0..4} ; do
	...
	if [ $? -eq 0 ] ;then
		...
		sleep 2
		echo "100"
		if [ $(nmcli d |grep "connected" -w | awk 'NR==2 {print $2}') != "wifi" ] ; then #Si la conexion no es por wifi lo desactiva
				nmcli r wifi off 
			fi
		break
