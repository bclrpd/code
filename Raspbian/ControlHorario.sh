...
while true ; do
	...
	if [ $? -eq 0 ] ; then
		...
		[ "$(xmlstarlet sel -t -v "count(/Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia])" Doc.xml)" -eq 0 ] && xmlstarlet ed -L \
			...
			-s /Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia] -t elem -n "TARDE" -v "" \
		...
		if [ $(date --date "$HoraAbrio" +%H%M) -lt 1300 ] ;then
			...
			[ -z "$(xmlstarlet sel -t -v /Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia]/MANANA/Hora Doc.xml)" ] || { echo "TIEMPO=0" > Data.ini && exit; }
			...
			if [ $(date --date "$HoraAbrio" +%H%M) -gt 0835 ] ;then
				...
				Mensage=$(yad --image="Imagen.jpg"  --geometry 1080x200+150+35 --image-on-top \
				...
				yad --no-buttons --undecorated --skip-taskbar --center  --width=300 --height=100 --back=red --timeout=5 \
				--timeout-indicator=top --text="Te exhortamos cumplir con el horario correspondiente y evitar incurrir en faltas en futuras ocasiones."
				
				echo "TIEMPO=0" > Data.ini
				echo "Banca=${Banca} Turno=1 Hora=$HoraAbrio Tardanza=False Razon=$Razon" | mutt -s "Banca ${Banca}" -- bancalarapidamail@gmail.com
			...
			[ -z "$(xmlstarlet sel -t -v /Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia]/TARDE/Hora Doc.xml)" ] || { echo "TIEMPO=0" > Data.ini && exit; }
			...
			if [ $(date --date "$HoraAbrio" +%H%M) -gt 1707 ] ;then
				...
				-u /Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia]/TARDE/Tardanza -v "True" \
				-u /Data/Year[@ID=$Year]/Mes[@ID=$Mes]/Dia[@ID=$Dia]/TARDE/Excuza -v "$Razon" \
				Doc.xml
				...
				echo "TIEMPO=0" > Data.ini
				echo "Banca=${Banca} Turno=2 Hora=$HoraAbrio Tardanza=False Razon=$Razon" | mutt -s "Banca ${Banca}" -- bancalarapidamail@gmail.com
				...
				echo "Banca=${Banca} Turno=2 Hora=$HoraAbrio Tardanza=False" | mutt -s "Banca ${Banca}" -- bancalarapidamail@gmail.com
			fi	
