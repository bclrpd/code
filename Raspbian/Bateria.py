...
async def bateria_weco(device):
    ...
    def decodificar_historial(data):
        ...
        fecha = datetime.fromtimestamp(int.from_bytes(data[58:62], "big")).strftime("%d-%m-%Y %H:%M:%S")
    ...
    def calcular_cantidad_registros(bateria_id):
        ...
        try:
            ...
            if response.status_code == 200:
                dato = response.json()['body']
                if not dato: return {'total': 144, 'fecha': datetime.now() - timedelta(days=2)}
        ...
        if device.name and any(item in device.name for item in BATERIAS_WECO):
            ...
            async with BleakClient(device) as client:
                ...
                if x.get('total', ''):
                    ...
                    datos_filtrados = [item for item in informacion if int(datetime.strptime(item['Fecha'], "%d-%m-%Y %H:%M:%S").strftime("%Y%m%d%H%M%S")) >= int(f)]
...
async def bateria_wltraton(device):
    ...
    def procesar_info(inf_basica, inf_celdas):
        ...
        data = {
            ...
            'Vol_C3': voltajes[2],
            'Vol_C4': voltajes[3],
            'Desbalanceo_mV': desbalanceo,
            'Temp': temp,
            'Ciclos': ciclos,
            'Mosfet_carga': mosfet_carga,
            'Mosfet_descarga': mosfet_descarga,
        }
        return data
    ...
    async with BleakClient(device) as client:
        if client.is_connected:
            ...
            try:
                ...
                guardar_estado(estado)
                intervalo = 300 if abs(estado['Corriente']) > 1 else 600
            except asyncio.TimeoutError:
